...
```
    ...
    ##     family = binomial, data = baltimore_lr)
    ## 
    ## Coefficients:
...
```
    ...
    ## ℹ In argument: `tidied = map(model, ~broom::tidy(.x, conf.int = TRUE))`.
...
while the second highest is tied between Fresno, CA, and Stockton, CA.
